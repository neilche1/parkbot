...
def identify_tenant(input_text):
    ...
    possible_park_name = " ".join(input_words)  # Reconstruct the input to check for park names
    ...
    if not possible_matches:
        for tenant_key in TENANTS:
            ...
            # Check for partial unit matches
            ...
            if input_text in full_name and tenant_key not in possible_matches:
                ...
                if all_words_present and tenant_key not in possible_matches:
                    ...
                    possible_matches.append(tenant_key)
                # Match first name and any part of the last name (e.g., "Clara Ines Lopez" matches "Clara Ines Wood Lopez")
...
def get_ai_response(user_input, tenant_data, is_maintenance_request=False):
    ...
    # Include all transactions if available, sorted by date
    transactions = tenant_data.get("transactions", [])
    # Check if transactions is None or empty before sorting
    if transactions:
        transactions.sort(key=lambda x: x.get("TransactionDate", ""), reverse=True)
    prompt = f"Tenant data: {tenant_data}. Tenant is from park: {park_details}. All transactions: {transactions}. Query: {user_input}"
    ...
    try:
        ...
        return "I’m sorry, I couldn’t process your request at this time. Please try again later or contact the park office at (504) 313-0024, available Monday to Friday, 9 AM to 5 PM."
...
def sms_reply():
    ...
    # Check if transaction data is needed and fetch it if not already present
    needs_transactions = (
    ...
    if "maintenance" in message_lower or "fix" in message_lower or "broken" in message_lower or "leak" in message_lower or "leaking" in message_lower or "flood" in message_lower or "damage" in message_lower or "repair" in message_lower or "clog" in message_lower or "power" in message_lower:
        ...
        # Handle specific queries manually if needed
        if "balance" in message_lower and "why" in message_lower:
            # Detailed breakdown of the balance using all transactions
            transactions = tenant_data["transactions"]
            if not transactions:
                reply = f"I’m sorry, I couldn’t retrieve your transaction history to explain your balance. Please contact the park office at {PARK_OFFICE_PHONE}, available {PARK_OFFICE_HOURS}, for assistance."
            else:
                # Categorize transactions
                charges = [t for t in transactions if t.get("TransactionType") != "Payment"]
                payments = [t for t in transactions if t.get("TransactionType") == "Payment"]
                
                # Sum charges by type
                rent_charges = [t for t in charges if "rent" in t.get("Comment", "").lower()]
                court_costs = [t for t in charges if "court" in t.get("Comment", "").lower() or "legal" in t.get("Comment", "").lower()]
                late_fees = [t for t in charges if "late fee" in t.get("Comment", "").lower()]
                other_fees = [t for t in charges if t not in rent_charges and t not in court_costs and t not in late_fees]
                
                total_rent = sum(float(t.get("Amount", 0.00)) for t in rent_charges)
                total_court_costs = sum(float(t.get("Amount", 0.00)) for t in court_costs)
                total_late_fees = sum(float(t.get("Amount", 0.00)) for t in late_fees)
                total_other_fees = sum(float(t.get("Amount", 0.00)) for t in other_fees)
                total_payments = sum(abs(float(t.get("Amount", 0.00))) for t in payments)
                
                # Match payments to charges to find unpaid items
                unpaid_rent = []
                unpaid_court_costs = []
                unpaid_late_fees = []
                unpaid_other_fees = []
                ...
                for charge in rent_charges:
                    amount = float(charge.get("Amount", 0.00))
                    if remaining_payment >= amount:
                        remaining_payment -= amount
                    else:
                        unpaid_rent.append(charge)
                
                # Process court costs
                for charge in court_costs:
                    amount = float(charge.get("Amount", 0.00))
                    if remaining_payment >= amount:
                        remaining_payment -= amount
                    else:
                        unpaid_court_costs.append(charge)
                
                # Process late fees
                for charge in late_fees:
                    amount = float(charge.get("Amount", 0.00))
                    if remaining_payment >= amount:
                        remaining_payment -= amount
                    else:
                        unpaid_late_fees.append(charge)
                
                # Process other fees
                for charge in other_fees:
                    amount = float(charge.get("Amount", 0.00))
                    if remaining_payment >= amount:
                        remaining_payment -= amount
                    else:
                        unpaid_other_fees.append(charge)
                
                # Build the explanation
                reply = f"Let me break down your balance of {tenant_data['balance']} for you:\n"
                reply += f"- Total rent charges since your move-in: ${total_rent:.2f}\n"
                if total_court_costs > 0:
                    reply += f"- Total court costs: ${total_court_costs:.2f}\n"
                if total_late_fees > 0:
                    reply += f"- Total late fees: ${total_late_fees:.2f}\n"
                if total_other_fees > 0:
                    reply += f"- Total other fees (e.g., utilities, maintenance): ${total_other_fees:.2f}\n"
                reply += f"- Total payments made: ${total_payments:.2f}\n"
                reply += "Here’s what remains unpaid:\n"
                if unpaid_rent:
                    reply += "- Unpaid rent:\n"
                    for charge in unpaid_rent:
                        date = charge.get("TransactionDate", "Unknown")
                        amount = float(charge.get("Amount", 0.00))
                        reply += f"  - Rent for {date}: ${amount:.2f}\n"
                if unpaid_court_costs:
                    reply += "- Unpaid court costs:\n"
                    for charge in unpaid_court_costs:
                        date = charge.get("TransactionDate", "Unknown")
                        amount = float(charge.get("Amount", 0.00))
                        reply += f"  - Court cost on {date}: ${amount:.2f}\n"
                if unpaid_late_fees:
                    reply += "- Unpaid late fees:\n"
                    for charge in unpaid_late_fees:
                        date = charge.get("TransactionDate", "Unknown")
                        amount = float(charge.get("Amount", 0.00))
                        reply += f"  - Late fee on {date}: ${amount:.2f}\n"
                if unpaid_other_fees:
                    reply += "- Unpaid other fees:\n"
                    for charge in unpaid_other_fees:
                        date = charge.get("TransactionDate", "Unknown")
                        amount = float(charge.get("Amount", 0.00))
                        comment = charge.get("Comment", "No comment")
                        reply += f"  - Fee on {date}: ${amount:.2f} ({comment})\n"
                if not (unpaid_rent or unpaid_court_costs or unpaid_late_fees or unpaid_other_fees):
                    reply += "- No specific unpaid charges identified; your balance may include adjustments or fees not categorized in the transactions.\n"
                reply += f"This results in your current balance of {tenant_data['balance']}.\n"
                reply += f"If this doesn’t look right, please contact the park office at {PARK_OFFICE_PHONE}, available {PARK_OFFICE_HOURS}, to confirm your payment records. You can also ask for a monthly statement to see your transactions. Would you like to know more?"
            send_sms(from_number, reply)
        elif "statement" in message_lower:
            # Generate a monthly statement for the last full month or a specific month if requested
            transactions = tenant_data["transactions"]
            if not transactions:
                reply = f"I’m sorry, I couldn’t retrieve your transaction history to generate a statement. Please contact the park office at {PARK_OFFICE_PHONE}, available {PARK_OFFICE_HOURS}, for assistance."
            else:
                # Check if a specific month is requested (e.g., "statement for January")
                ...
                for month_name, month_num in months.items():
                    if f"for {month_name}" in message_lower:
                        requested_month = month_name
                        break
                
                today = datetime.datetime.now()
                if requested_month:
                    # Determine the year (assume current year unless specified)
                    year = today.year
                    # Check if a year is specified (e.g., "statement for January 2024")
                    year_match = re.search(r"for \w+\s+(\d{4})", message_lower)
                    if year_match:
                        year = int(year_match.group(1))
                    month_num = months[requested_month]
                    start_date = datetime.datetime(year, month_num, 1)
                    # Calculate the end date as the last day of the requested month
                    next_month = month_num + 1 if month_num < 12 else 1
                    next_year = year if month_num < 12 else year + 1
                    end_date = datetime.datetime(next_year, next_month, 1) - datetime.timedelta(days=1)
                    period_label = f"{requested_month.capitalize()} {year}"
                else:
                    # Default to the last full month
                    last_month = today.month - 1 if today.month > 1 else 12
                    year = today.year if today.month > 1 else today.year - 1
                    start_date = datetime.datetime(year, last_month, 1)
                ...
                monthly_transactions = [t for t in transactions if start_str <= t.get("TransactionDate", "") <= end_str]

                if not monthly_transactions:
                    reply = f"I don’t have any transactions recorded for {period_label}. If you believe there’s an error, please contact the park office at {PARK_OFFICE_PHONE}, available {PARK_OFFICE_HOURS}, to confirm. Would you like assistance with anything else?"
                else:
                    reply = f"Here’s your statement for {period_label}:\n"
                    running_balance = 0.0
                    for t in monthly_transactions:
                        date = t.get("TransactionDate", "Unknown")
                        amount = float(t.get("Amount", 0.00))
                        comment = t.get("Comment", "No comment")
                        if t.get("TransactionType") == "Payment":
                            amount = abs(amount)  # Payments are negative in the API
                            running_balance -= amount
                            reply += f"- {date}: Payment of ${amount:.2f} ({comment}) | Balance: ${running_balance:.2f}\n"
                        else:
                            running_balance += amount
                            reply += f"- {date}: Charge of ${amount:.2f} ({comment}) | Balance: ${running_balance:.2f}\n"
                    
                    # Categorize transactions for summary
                    charges = [t for t in monthly_transactions if t.get("TransactionType") != "Payment"]
                    payments = [t for t in monthly_transactions if t.get("TransactionType") == "Payment"]
                    ...
                    reply += f"- Total rent charges: ${total_rent:.2f}\n"
                    if total_court_costs > 0:
                        reply += f"- Total court costs: ${total_court_costs:.2f}\n"
                    if total_late_fees > 0:
                        reply += f"- Total late fees: ${total_late_fees:.2f}\n"
                    if total_other_fees > 0:
                        reply += f"- Total other fees: ${total_other_fees:.2f}\n"
                    reply += f"- Total payments: ${total_payments:.2f}\n"
                    reply += f"- Ending balance for {period_label}: ${running_balance:.2f}\n"
                    reply += f"Note: Your current overall balance is {tenant_data['balance']}, which includes transactions from other months. If you have any questions, please contact the park office at {PARK_OFFICE_PHONE}, available {PARK_OFFICE_HOURS}. Would you like to see a statement for a different month?"
                send_sms(from_number, reply)
        elif "balance" in message_lower:
            last_payment_date = tenant_data["last_payment_date"]
            if last_payment_date != "Unknown":
                try:
                    date_obj = datetime.datetime.strptime(last_payment_date, "%Y-%m-%dT%H:%M:%S")
                    formatted_date = date_obj.strftime("%B %d, %Y")
                    reply = f"Your current balance is {tenant_data['balance']}, due by the {tenant_data['due_date']} of the month. Your last payment was received on {formatted_date}. If you’d like a detailed breakdown of your balance, just ask! Is there anything else I can assist you with?"
                except ValueError:
                    reply = f"Your current balance is {tenant_data['balance']}, due by the {tenant_data['due_date']} of the month. I couldn’t determine the exact date of your last payment, but I can provide more details if needed. If you’d like a detailed breakdown of your balance, just ask! Is there anything else I can assist you with?"
            else:
                reply = f"Your current balance is {tenant_data['balance']}, due by the {tenant_data['due_date']} of the month. I don’t have a record of your last payment date. If you’d like a detailed breakdown of your balance, just ask! Is there anything else I can assist you with?"
            send_sms(from_number, reply)
        elif "pay" in message_lower or "due" in message_lower:
            # Get the park-specific details from the tenant's data
            park_details = tenant_data["park"]
            # Check if the query is specifically about how to pay
            if "how" in message_lower:
                reply = (
                    f"You can pay your rent of {tenant_data['balance']}, due by the {tenant_data['due_date']} of the month, "
                    f"{park_details['payment_methods']}. Please make your payment payable to {park_details['payee']} "
                    f"and {park_details['payment_procedure']}. If you have any questions, feel free to let me know! "
                ...
                if last_payment_date != "Unknown":
                    try:
                        date_obj = datetime.datetime.strptime(last_payment_date, "%Y-%m-%dT%H:%M:%S")
                        formatted_date = date_obj.strftime("%B %d, %Y")
                        reply = f"You have to pay your balance of {tenant_data['balance']} by the {tenant_data['due_date']} of the month. Your last payment was received on {formatted_date}. If you need payment options or have questions, let me know! Is there anything else I can assist you with?"
                    except ValueError:
                        reply = f"You have to pay your balance of {tenant_data['balance']} by the {tenant_data['due_date']} of the month. I couldn’t determine the exact date of your last payment, but I can provide more details if needed. If you need payment options or have questions, let me know! Is there anything else I can assist you with?"
                else:
                    reply = f"You have to pay your balance of {tenant_data['balance']} by the {tenant_data['due_date']} of the month. I don’t have a record of your last payment date. If you need payment options or have questions, let me know! Is there anything else I can assist you with?"
                send_sms(from_number, reply)
        elif "payment history" in message_lower or "last payment" in message_lower or "recent transactions" in message_lower:
            # Provide a summary of the tenant's recent payments
            transactions = tenant_data["transactions"]
            if not transactions:
                reply = f"I’m sorry, I couldn’t retrieve your transaction history at this time. Please contact the park office at {PARK_OFFICE_PHONE}, available {PARK_OFFICE_HOURS}, for assistance. Is there anything else I can assist you with?"
            else:
                payment_transactions = [t for t in transactions if t.get("TransactionType") == "Payment"]
                # Sort in descending order for recent transactions
                payment_transactions.sort(key=lambda x: x.get("TransactionDate", ""), reverse=True)
                if payment_transactions:
                    # Show the 3 most recent payments for readability in SMS
                    recent_payments = payment_transactions[:3]
                    payment_details = []
                    for payment in recent_payments:
                        date = payment.get("TransactionDate", "Unknown")
                        amount = abs(float(payment.get("Amount", 0.00)))
                        comment = payment.get("Comment", "No comment")
                        try:
                            date_obj = datetime.datetime.strptime(date, "%Y-%m-%dT%H:%M:%S")
                            formatted_date = date_obj.strftime("%B %d, %Y")
                            payment_details.append(f"- Payment of ${amount:.2f} on {formatted_date} ({comment})")
                        except ValueError:
                            payment_details.append(f"- Payment of ${amount:.2f} on an unknown date ({comment})")
                    total_payments = len(payment_transactions)
                    reply = f"Here’s a summary of your 3 most recent payments (out of {total_payments} total payments):\n" + "\n".join(payment_details) + "\n"
                    # Check for multiple payments on the same day
                    payment_dates = {}
                    for payment in payment_transactions:
                        date = payment.get("TransactionDate", "Unknown")
                        payment_dates[date] = payment_dates.get(date, 0) + 1
                    multiple_payments = [date for date, count in payment_dates.items() if count > 1]
                    if multiple_payments:
                        reply += f"I noticed multiple payments on the same day (e.g., {multiple_payments[0]}). If this looks incorrect, please contact the park office at {PARK_OFFICE_PHONE}, available {PARK_OFFICE_HOURS}, to confirm.\n"
                    reply += "Would you like more details or assistance with something else?"
                else:
                    reply = f"I don’t have any payment records for you at the moment. If you have questions about your payment history, please contact the park office at {PARK_OFFICE_PHONE}, available {PARK_OFFICE_HOURS}, for more details. Is there anything else I can assist you with?"
            send_sms(from_number, reply)
        elif "last month" in message_lower:
            # Provide a summary of transactions for the last month
            transactions = tenant_data["transactions"]
            if not transactions:
                reply = f"I’m sorry, I couldn’t retrieve your transaction history for the last month. Please contact the park office at {PARK_OFFICE_PHONE}, available {PARK_OFFICE_HOURS}, for assistance. Is there anything else I can assist you with?"
            else:
                # Define the date range for the last month
                today = datetime.datetime.now()
                start_date = (today - datetime.timedelta(days=30)).strftime("%Y-%m-%d")
                end_date = today.strftime("%Y-%m-%d")
                last_month_transactions = [t for t in transactions if start_date <= t.get("TransactionDate", "") <= end_date]
                
                if not last_month_transactions:
                    reply = f"I don’t have any transactions recorded for you in the last month (from {start_date} to {end_date}). Your current balance is {tenant_data['balance']}, due on the {tenant_data['due_date']} of the month. If you believe a payment is missing, please contact the park office at {PARK_OFFICE_PHONE}, available {PARK_OFFICE_HOURS}, to confirm. Is there anything else I can assist you with?"
                else:
                    charges = [t for t in last_month_transactions if t.get("TransactionType") != "Payment"]
                    payments = [t for t in last_month_transactions if t.get("TransactionType") == "Payment"]
                    total_charges = sum(float(t.get("Amount", 0.00)) for t in charges)
                    total_payments = sum(abs(float(t.get("Amount", 0.00))) for t in payments)
                    
                    reply = f"Here’s a summary of your transactions for the last month (from {start_date} to {end_date}):\n"
                    if charges:
                        reply += f"- Total charges: ${total_charges:.2f} (e.g., a charge of ${charges[0].get('Amount', 0.00)} on {charges[0].get('TransactionDate', 'unknown date')})\n"
                    else:
                        reply += "- No charges recorded.\n"
                    if payments:
                        reply += f"- Total payments: ${total_payments:.2f}\n"
                        # Show up to 2 payments as examples
                        for payment in payments[:2]:
                            date = payment.get("TransactionDate", "Unknown")
                            amount = abs(float(payment.get("Amount", 0.00)))
                            reply += f"  - Payment of ${amount:.2f} on {date} ({payment.get('Comment', 'No comment')})\n"
                        # Check for multiple payments on the same day
                        payment_dates = {}
                        ...
                        reply += "- No payments recorded.\n"
                    reply += f"Your current balance is {tenant_data['balance']}, due on the {tenant_data['due_date']} of the month. Would you like a detailed breakdown of your balance or more transaction details?"
                send_sms(from_number, reply)
