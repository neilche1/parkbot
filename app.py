...
def fetch_tenants_from_rent_manager():
    ...
    for api_url in RENT_MANAGER_API_URLS:
        ...
        # Try with X-RM12Api-Token header first (corrected header name)
        headers = {
            "X-RM12Api-Token": RENT_MANAGER_API_TOKEN,  # Corrected header name
            "Content-Type": "application/json; charset=UTF-8"
        }
        try:
            print(f"Fetching tenants with X-RM12Api-Token header: {headers}")
            response = requests.get(api_url, headers=headers)
            print(f"Tenant Fetch Response Status (X-RM12Api-Token, {api_url}): {response.status_code}")
            print(f"Tenant Fetch Response Text (X-RM12Api-Token, {api_url}): {response.text}")
            print(f"Tenant Fetch Response Headers (X-RM12Api-Token, {api_url}): {response.headers}")
            response.raise_for_status()
            tenants_data = response.json()
            break  # If successful, break the loop
        except requests.exceptions.RequestException as e:
            print(f"Error fetching tenants with X-RM12Api-Token at {api_url}: {str(e)}")
            if response.status_code == 401:
                ...
                try:
                    print(f"Retrying tenants fetch with Authorization: Bearer header: {headers}")
                    response = requests.get(api_url, headers=headers)
