...
def fetch_tenants_from_rent_manager():
    ...
    all_tenants = []
    url = "https://shadynook.api.rentmanager.com/Tenants?embeds=Property,Property.Addresses,Addresses,Leases.Unit.UnitType,Balance&filters=Status,eq,Current"
    ...
    for tenant in all_tenants:
        ...
        tenants[tenant_key] = {
            ...
            "park": park  # Add the entire park object to the tenant's data
        }
...
def identify_tenant(input_text):
    ...
    # First, try to identify a park name in the input
    ...
    for tenant_key in TENANTS:
        ...
        # Normalize the unit by removing spaces for comparison
        unit_normalized = unit_lower.replace(" ", "")
        tenant_park_name = TENANTS[tenant_key]["park"]["name"].lower()

        print(f"Checking tenant: TenantID={tenant_id}, FullName='{full_name}', FirstName='{first_name_lower}', LastName='{last_name_lower}', Unit='{unit_lower}', Park='{tenant_park_name}'")

        # Check for matches
        # Match by unit number (exact match after normalization)
        ...
        # Match by unit number (partial match, but only if the input starts with the unit)
        elif input_text_normalized.startswith(unit_normalized):
    ...
    # If a park name was identified in the input, filter matches to only include tenants from that park
...
def get_ai_response(user_input, tenant_data, is_maintenance_request=False):
    # Include the tenant's park details in the prompt
    ...
    prompt = f"Tenant data: {tenant_data}. Tenant is from park: {park_details}. Query: {user_input}"
            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        "For other queries, respond helpfully and professionally using the park-specific details provided (e.g., payment methods, payment procedure, payee). "
...
def sms_reply():
    ...
    if "maintenance" in message_lower or "fix" in message_lower or "broken" in message_lower or "leak" in message_lower or "leaking" in message_lower or "flood" in message_lower or "damage" in message_lower or "repair" in message_lower or "clog" in message_lower or "power" in message_lower:
        ...
        if "balance" in message_lower:
            reply = f"Your current balance is {tenant_data['balance']}, due by the {tenant_data['due_date']} of the month. If you have any questions about your balance, feel free to ask!"
            reply += " Is there anything else I can assist you with?"
            send_sms(from_number, reply)
            ...
            if "how" in message_lower:
                ...
                reply = f"You have to pay your balance of {tenant_data['balance']} by the {tenant_data['due_date']} of the month. If you need payment options or have questions, let me know!"
                reply += " Is there anything else I can assist you with?"
                send_sms(from_number, reply)
