...
# Fetch tenant data from Rent Manager API with pagination, filtering for active tenants (Status="Current")
def fetch_tenants_from_rent_manager():
    ...
    # Filter active tenants (Status="Current") and process them into the required format
    tenants = {}
    active_tenants = 0
    inactive_tenants = 0
    ...
    for tenant in all_tenants:
        ...
        # Check if the tenant is active based on Status
        status = tenant.get("Status", "Unknown")
        if status != "Current":
            print(f"Skipping inactive tenant: TenantID={tenant_id}, Name='{tenant.get('Name', 'Unknown')}', Status={status}")
            inactive_tenants += 1
            continue
        ...
        print(f"Stored tenant: TenantID={tenant_id}, Name='{name}', Lot='{lot}', Status={status}")

    print(f"Successfully fetched {len(tenants)} active tenants from Rent Manager (total tenants: {len(all_tenants)}, active: {active_tenants}, inactive: {inactive_tenants})")
    return tenants
    ...
    if from_number not in CURRENT_CONVERSATIONS:
        if from_number in PENDING_IDENTIFICATION:
            ...
            if tenant_key:
                ...
                # Process the pending message if it exists
                if pending_message:
                    ...
                    send_sms(from_number, "Hello Clara! I’ve identified you. How can I assist you today?")
                return "OK"
            else:
