...
def get_ai_response(user_input, tenant_data):
    ...
    try:
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "You are a mobile home park manager assisting tenants. For maintenance requests, confirm that the issue has been logged and the owner has been notified, and provide a professional response."},
...
def sms_reply():
    ...
    if tenant_data:
        ...
        if "maintenance" in message_lower or "fix" in message_lower or "broken" in message_lower or "leak" in message_lower or "leaking" in message_lower:
