...
RENT_MANAGER_BASE_URL = "https://shadynook.api.rentmanager.com/Tenants?embeds=Property,Property.Addresses,Addresses,Leases.Unit.UnitType,Balance,Transactions&filters=Status,eq,Current"
...
def fetch_tenants_from_rent_manager():
    ...
    # Parameters for the initial request with updated embeds
    params = {
    ...
    for tenant in all_tenants:
        ...
        transactions = tenant.get("Transactions", [])
        ...
        tenants[tenant_key] = {
            ...
            "last_payment_date": last_payment_date  # Store the most recent payment date
        }
...
def get_ai_response(user_input, tenant_data, is_maintenance_request=False):
    ...
    # Include only the 5 most recent transactions to avoid overwhelming the prompt
    recent_transactions = tenant_data.get("transactions", [])[:5]
    prompt = f"Tenant data: {tenant_data}. Tenant is from park: {park_details}. Recent transactions: {recent_transactions}. Query: {user_input}"
            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        "Use the tenant's recent transactions, provided as 'Recent transactions: {recent_transactions}', to inform your responses when relevant, such as for payment-related queries. "
                        ...
                        "For other queries, respond helpfully and professionally using the park-specific details provided (e.g., payment methods, payment_procedure, payee). "
    ...
    if "maintenance" in message_lower or "fix" in message_lower or "broken" in message_lower or "leak" in message_lower or "leaking" in message_lower or "flood" in message_lower or "damage" in message_lower or "repair" in message_lower or "clog" in message_lower or "power" in message_lower:
        ...
        if "balance" in message_lower:
            ...
            if payment_transactions:
                # Sort by TransactionDate in descending order
                payment_transactions.sort(key=lambda x: x.get("TransactionDate", ""), reverse=True)
                # Get the 3 most recent payments (or fewer if there aren't 3)
