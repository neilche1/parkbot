...
def sms_reply():
    ...
    if from_number in CURRENT_CONVERSATIONS:
        ...
        print(f"Conversation active for {from_number}, time since last message: {time_delta} minutes")
        ...
        if time_delta >= 5 and not CURRENT_CONVERSATIONS[from_number].get("pending_end", False):
            ...
            print(f"Inactivity timeout triggered for {from_number}")
            return "OK"
        ...
        if CURRENT_CONVERSATIONS[from_number].get("pending_end", False):
            ...
            if end_delta >= 3:
                ...
                print(f"Conversation closed for {from_number} due to no response after end prompt")
                return "OK"
        ...
        if from_number in PENDING_IDENTIFICATION:
            ...
            if tenant_key:
                ...
                if possible_matches:
                    ...
                    tenant_names = [f"{match[1]} W {match[2]}" for match in possible_matches]
