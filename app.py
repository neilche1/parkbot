...
def fetch_tenants_from_rent_manager():
    ...
    for tenant in all_tenants:
        ...
        if addresses and isinstance(addresses, list) and len(addresses) > 0:
            # Try to get the unit from the Unit field
            lot = addresses[0].get("Unit", "Unknown")
            # If Unit is not available or empty, try to extract from Street field
            if lot == "Unknown" or not lot or lot.strip() == "":
                street = addresses[0].get("Street", "")
                # Look for "Lot # [number]" in the Street field
                lot_match = re.search(r"Lot #\s*(\d+)", street, re.IGNORECASE)
                if lot_match:
                    lot = lot_match.group(1)  # Extract just the number (e.g., "21")
        ...
        # Extract address details for later use
        address_details = {
...
def check_inactive_conversations():
    ...
    for phone_number, conversation in list(CURRENT_CONVERSATIONS.items()):
        ...
        if time_delta >= 5 and not conversation.get("pending_end", False):
    ...
    if "maintenance" in message_lower or "fix" in message_lower or "broken" in message_lower or "leak" in message_lower or "leaking" in message_lower or "flood" in message_lower or "damage" in message_lower or "repair" in message_lower or "clog" in message_lower or "power" in message_lower:
        ...
        if "balance" in message_lower:
            ...
            if "how" in message_lower:
                reply = f"You can pay your rent of {tenant_data['balance']}, due by the {tenant_data['due_date']} of the month, by mailing a check to the park office at 123 Park Lane, Anytown, USA 12345, or by dropping off a payment in person at the office during business hours (Monday-Friday, 9 AM to 5 PM). Please make checks payable to Shady Nook Mobile Home Park. If you have any questions or need other payment options, feel free to let me know!"
                reply += " Is there anything else I can assist you with?"
                send_sms(from_number, reply)
            else:
