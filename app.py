...
def fetch_tenants_from_rent_manager():
    ...
    batch_size = 50  # Process tenants in batches to manage memory usage
    batch_count = 0
    ...
    for tenant in all_tenants:
        try:
            ...
            # Get the primary address of the park
            ...
            print(f"Stored tenant: TenantID={tenant_id}, Name='{name}', Lot='{lot}', Park='{park['name']}', Status=Current")
            
            batch_count += 1
            # Process in batches to manage memory usage
            if batch_count >= batch_size:
                print(f"Processed batch of {batch_count} tenants. Total stored: {len(tenants)}")
                batch_count = 0
...
def fetch_tenant_transactions(tenant_id):
    ...
    try:
        ...
        # Sort transactions by TransactionDate to find the most recent payment
...
# Initialize tenant data as an empty dictionary (will be populated in background)
TENANTS = {}

# Background task to fetch tenants
def fetch_tenants_background():
    global TENANTS
    print("Starting background task to fetch tenants...")
    TENANTS = fetch_tenants_from_rent_manager()
    print("Background task to fetch tenants completed.")

# Start the background task to fetch tenants
threading.Thread(target=fetch_tenants_background, daemon=True).start()
...
def identify_tenant(input_text):
    # Check if TENANTS is still being populated
    if not TENANTS:
        print("Tenant data is still being fetched. Please wait a moment and try again.")
        return None, None
...
def get_ai_response(user_input, tenant_data, is_maintenance_request=False):
    ...
    transactions = tenant_data.get("transactions", [])
    # Sort transactions by date to ensure the most recent are prioritized
            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        "If you lack specific details to answer a query, respond with: 'I’m sorry, I don’t have that information. Please contact the park office for more details.'"
                    )
                },
...
def sms_reply():
    ...
    if needs_transactions and tenant_data["transactions"] is None:
        ...
        if transactions is not None:
            ...
            send_sms(from_number, "I’m sorry, I couldn’t retrieve your transaction data at this time. Please try again later or contact the park office for assistance.")
            return "OK"
        ...
        if "balance" in message_lower:
            ...
            if last_payment_date != "Unknown":
                try:
                    ...
                    reply = f"Your current balance is {tenant_data['balance']}, due by the {tenant_data['due_date']} of the month. Your last payment was received on {formatted_date}. If you have any questions about your balance, feel free to ask!"
                except ValueError:
                    reply = f"Your current balance is {tenant_data['balance']}, due by the {tenant_data['due_date']} of the month. I couldn’t determine the exact date of your last payment, but I can provide more details if needed. If you have any questions about your balance, feel free to ask!"
            else:
                reply = f"Your current balance is {tenant_data['balance']}, due by the {tenant_data['due_date']} of the month. I don’t have a record of your last payment date. If you have any questions about your balance, feel free to ask!"
            reply += " Is there anything else I can assist you with?"
            send_sms(from_number, reply)
            ...
            if "how" in message_lower:
                ...
                if last_payment_date != "Unknown":
                    try:
                        date_obj = datetime.datetime.strptime(last_payment_date, "%Y-%m-%dT%H:%M:%S")
                        formatted_date = date_obj.strftime("%B %d, %Y")
                        reply = f"You have to pay your balance of {tenant_data['balance']} by the {tenant_data['due_date']} of the month. Your last payment was received on {formatted_date}. If you need payment options or have questions, let me know!"
                    except ValueError:
                        reply = f"You have to pay your balance of {tenant_data['balance']} by the {tenant_data['due_date']} of the month. I couldn’t determine the exact date of your last payment, but I can provide more details if needed. If you need payment options or have questions, let me know!"
                else:
                    reply = f"You have to pay your balance of {tenant_data['balance']} by the {tenant_data['due_date']} of the month. I don’t have a record of your last payment date. If you need payment options or have questions, let me know!"
                reply += " Is there anything else I can assist you with?"
                send_sms(from_number, reply)
        elif "payment history" in message_lower or "last payment" in message_lower:
            ...
            if payment_transactions:
                ...
                for payment in recent_payments:
                    ...
                    try:
                        ...
                        formatted_date = date_obj.strftime("%B %d, %Y")
                        payment_details.append(f"On {formatted_date}, you paid ${abs(float(amount)):.2f} ({comment})")
                    except ValueError:
                        payment_details.append(f"On an unknown date, you paid ${abs(float(amount)):.2f} ({comment})")
                ...
                reply = "I don’t have any payment records for you at the moment. If you have questions about your payment history, you can contact the park office for more details. Is there anything else I can assist you with?"
            send_sms(from_number, reply)
        elif "address" in message_lower:
            address = tenant_data['address']
            full_address = f"{address['street']}, {address['city']}, {address['state']} {address['postal_code']}"
            reply = f"Your primary address on file is {full_address}."
            send_sms(from_number, reply)
        elif "unit" in message_lower or "lot" in message_lower:
            reply = f"Your unit number is {tenant_key[3]}."
            send_sms(from_number, reply)
            ...
            if move_in_date != "Unknown":
                ...
                try:
                    ...
                    formatted_date = date_obj.strftime("%B %d, %Y")
                    reply = f"Your move-in date was {formatted_date}. If you need any more information, feel free to ask!"
                except ValueError:
                    reply = "I’m sorry, I couldn’t determine your move-in date. Please contact the park office for more details."
            else:
                reply = "I’m sorry, I couldn’t determine your move-in date. Please contact the park office for more details."
            send_sms(from_number, reply)
        elif "phone number" in message_lower:
            reply = "I'm here to help with any questions you have. For your phone number, you can find it on your lease agreement or by checking any recent communication from the park office. If you need further assistance, feel free to ask!"
            reply += " Is there anything else I can assist you with?"
            send_sms(from_number, reply)
        else:
