...
def get_ai_response(user_input, tenant_data, is_maintenance_request=False):
    ...
    logger.info(f"Sending {len(transactions)} transactions to xAI API for tenant {tenant_data.get('tenant_id', 'Unknown')}")
    ...
    def call_xai():
        ...
        try:
            ...
            response = requests.post("https://api.x.ai/v1/chat/completions", headers=headers, json=payload, timeout=20)
            response.raise_for_status()
