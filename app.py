...
# Fetch tenant data from Rent Manager API with pagination, filtering for active tenants in code
def fetch_tenants_from_rent_manager():
    ...
    # Parameters for the initial request (remove IsActive since it's not working)
    params = {
    ...
    # Filter active tenants and process them into the required format
    tenants = {}
    active_tenants = 0
    inactive_tenants = 0
    for tenant in all_tenants:
        ...
        is_active = tenant.get("IsActive", False)
        if not is_active:
            print(f"Skipping inactive tenant: TenantID={tenant_id}, Name='{tenant.get('Name', 'Unknown')}'")
            inactive_tenants += 1
            continue
        ...
        print(f"Stored tenant: TenantID={tenant_id}, Name='{name}', Lot='{lot}', IsActive={is_active}")
