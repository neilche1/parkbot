...
RENT_MANAGER_API_URLS = [
    "https://shadynook.api.rentmanager.com/Tenants",
    "https://shadynook.api.rentmanager.com/Tenants/list",
    "https://shadynook.api.rentmanager.com/v1/Tenants",
    "https://shadynook.api.rentmanager.com/api/v1/Tenants"
]
...
def authenticate_with_rent_manager():
    ...
    RENT_MANAGER_API_TOKEN = "3dY4Xu1d7MBR0R3pDj3uYdDxdK79Dw6mTHev2e6cEIgNw293VTOGq_Dcr4fifwhtX88Pna6n6yX1LDg1uIplaRwJdZdMPkTWVGw0-yPAvDY="
    print(f"Using hardcoded token for testing: {RENT_MANAGER_API_TOKEN}")
...
def fetch_tenants_from_rent_manager():
    ...
    # Add LocationID as a query parameter
    params = {
        "LocationID": RENT_MANAGER_LOCATION_ID
    }
    # Try different endpoints
    for api_url in RENT_MANAGER_API_URLS:
        print(f"Trying endpoint: {api_url}")

        # Use the corrected header name X-RM12Api-ApiToken
        headers = {
            "X-RM12Api-ApiToken": RENT_MANAGER_API_TOKEN,  # Corrected header name
            "Content-Type": "application/json; charset=UTF-8",
            "Accept": "application/json",
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36"
        }
        try:
            print(f"Fetching tenants with X-RM12Api-ApiToken header: {headers}, params: {params}")
            response = requests.get(api_url, headers=headers, params=params)
            print(f"Tenant Fetch Response Status (X-RM12Api-ApiToken, {api_url}): {response.status_code}")
            print(f"Tenant Fetch Response Text (X-RM12Api-ApiToken, {api_url}): {response.text}")
            print(f"Tenant Fetch Response Headers (X-RM12Api-ApiToken, {api_url}): {response.headers}")
            response.raise_for_status()
            tenants_data = response.json()
            break  # If successful, break the loop
        except requests.exceptions.RequestException as e:
            print(f"Error fetching tenants with X-RM12Api-ApiToken at {api_url}: {str(e)}")
            if response.status_code == 401:
                # Try with Authorization: Bearer header as a fallback
                headers = {
                    "Authorization": f"Bearer {RENT_MANAGER_API_TOKEN}",
                    "Content-Type": "application/json; charset=UTF-8",
                    "Accept": "application/json",
                    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36"
                }
                try:
                    print(f"Retrying tenants fetch with Authorization: Bearer header: {headers}, params: {params}")
                    response = requests.get(api_url, headers=headers, params=params)
                    print(f"Tenant Fetch Response Status (Authorization, {api_url}): {response.status_code}")
                    print(f"Tenant Fetch Response Text (Authorization, {api_url}): {response.text}")
                    print(f"Tenant Fetch Response Headers (Authorization, {api_url}): {response.headers}")
                    response.raise_for_status()
                    tenants_data = response.json()
                    break  # If successful, break the loop
                except requests.exceptions.RequestException as e2:
                    print(f"Error fetching tenants with Authorization: Bearer at {api_url}: {str(e2)}")
                    continue  # Try the next endpoint
            else:
                continue  # Try the next endpoint

    # If all attempts fail, return empty tenants
    if 'tenants_data' not in locals():
        print("All attempts to fetch tenants failed.")
        return {}
