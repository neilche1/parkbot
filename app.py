...
# Function to check for inactive conversations and close them
    ...
    while True:
        ...
        for phone_number, conversation in list(CURRENT_CONVERSATIONS.items()):
            ...
            if time_delta >= 5 and not conversation.get("pending_end", False):
                ...
                print(f"Inactivity timeout triggered for {phone_number} by background task")
        ...
        for phone_number in conversations_to_close:
            if phone_number in CURRENT_CONVERSATIONS:
                ...
                print(f"Conversation closed for {phone_number} by background task due to no response after end prompt")
...
def start_background_task():
    thread = threading.Thread(target=check_inactive_conversations, daemon=True)
    thread.start()
...
def sms_reply():
    ...
    if "maintenance" in message_lower or "fix" in message_lower or "broken" in message_lower or "leak" in message_lower or "leaking" in message_lower or "flood" in message_lower or "damage" in message_lower or "repair" in message_lower or "clog" in message_lower or "power" in message_lower:
        ...
        if "balance" in message_lower:
            reply = f"Your current balance is {tenant_data['balance']}. If you have any questions about your balance, feel free to ask!"
