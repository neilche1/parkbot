...
def fetch_tenants_from_rent_manager():
    ...
    for tenant in all_tenants:
        ...
        # Fetch the balance using the /Tenants/{id}/Ledger endpoint
        balance = 0.00
        try:
            ledger_url = f"https://shadynook.api.rentmanager.com/Tenants/{tenant_id}/Ledger"
            ...
            # Calculate the balance by summing charges and subtracting payments
            for entry in ledger_data:
                amount = float(entry.get("Amount", 0.00))
                transaction_type = entry.get("TransactionType", "").lower()
                # Charges increase the balance, payments decrease it
                if "charge" in transaction_type:
                    balance += amount
