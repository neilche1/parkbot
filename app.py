...
def get_ai_response(user_input, tenant_data, is_maintenance_request=False):
    ...
    tenant_data_str = json.dumps(tenant_data)
    ...
    @retry(
        ...
        wait=wait_exponential(multiplier=1, min=5, max=10),  # Exponential backoff: 5s, 10s
    ...
    def call_xai():
        ...
        payload = {
            ...
            "model": "grok-3-fast-beta",  # Revert to grok-3-fast-beta for better accuracy
            "stream": False,
            "temperature": 0.5,
            "max_tokens": 500
        }
