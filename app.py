...
RENT_MANAGER_BASE_URL = "https://shadynook.api.rentmanager.com/Tenants?filters=Status,eq,Current"
...
def fetch_tenants_from_rent_manager():
    ...
    for tenant in all_tenants:
        ...
        # Fetch the unit number using the UnitID
        unit_id = tenant.get("UnitID")
        lot = "Unknown"
        if unit_id:
            try:
                unit_url = f"https://shadynook.api.rentmanager.com/Units/{unit_id}"
                print(f"Fetching unit details from {unit_url} for TenantID={tenant_id}")
                unit_response = requests.get(unit_url, headers=headers)
                unit_response.raise_for_status()
                unit_data = unit_response.json()
                # The unit number might be in "UnitNumber" or "Name" field; try both
                lot = unit_data.get("UnitNumber", unit_data.get("Name", "Unknown"))
                if not lot or lot.strip() == "":
                    lot = "Unknown"
                    print(f"TenantID {tenant_id} has missing or empty UnitNumber/Name field, using 'Unknown'")
            except requests.exceptions.RequestException as e:
                print(f"Error fetching unit details for UnitID {unit_id}: {str(e)}")
                lot = "Unknown"

        # Fetch the balance using the /Tenants/{id}/Ledger endpoint with a date range
        balance = 0.00
        try:
            # Set a date range to fetch all transactions up to today (e.g., from 2020-01-01 to today)
            today = datetime.now().strftime("%Y-%m-%d")
            ledger_url = f"https://shadynook.api.rentmanager.com/Tenants/{tenant_id}/Ledger?startDate=2020-01-01&endDate={today}"
            print(f"Fetching ledger for TenantID={tenant_id} from {ledger_url}")
            ledger_response = requests.get(ledger_url, headers=headers)
            ledger_response.raise_for_status()
            ledger_data = ledger_response.json()
            print(f"Ledger data for TenantID={tenant_id}: {ledger_data[:5]}...")  # Log the first 5 entries for debugging
            # Calculate the balance by considering various transaction types
            for entry in ledger_data:
                amount = float(entry.get("Amount", 0.00))
                transaction_type = entry.get("TransactionType", "").lower()
                print(f"Processing ledger entry for TenantID={tenant_id}: TransactionType={transaction_type}, Amount={amount}")
                # Transactions that increase the balance (tenant owes more)
                if any(keyword in transaction_type for keyword in ["charge", "rent", "late fee", "fee"]):
                    balance += amount
                # Transactions that decrease the balance (tenant pays or gets a credit)
                elif any(keyword in transaction_type for keyword in ["payment", "credit"]):
                    balance -= amount
            balance = f"${balance:.2f}"
        except requests.exceptions.RequestException as e:
            print(f"Error fetching ledger for TenantID {tenant_id}: {str(e)}")
            balance = f"${float(tenant.get('Balance', 0.00)):.2f}"  # Fallback to the /Tenants Balance field
        ...
        print(f"Stored tenant: TenantID={tenant_id}, Name='{name}', Lot='{lot}', Balance={balance}, Status=Current")
