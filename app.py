...
def identify_tenant(input_text):
    ...
    logger.info(f"Attempting to identify tenant with input: '{input_text}'")
    ...
    for tenant_key in TENANTS:
        ...
        logger.debug(f"Checking tenant: TenantID={tenant_id}, FullName='{full_name}', FirstName='{first_name_lower}', LastName='{last_name_lower}', Unit='{unit_lower}', Park='{tenant_park_name}', City='{tenant_city}'")
...
def get_ai_response(user_input, tenant_data, is_maintenance_request=False):
    ...
    transaction_tokens = len(transactions) * 10  # ~10 tokens per transaction
    prompt_base_tokens = len(str(tenant_data)) + len(str(park_details)) + len(user_input)  # Rough estimate
    total_input_tokens = transaction_tokens + prompt_base_tokens
    logger.info(f"Sending {len(transactions)} transactions to xAI API for tenant {tenant_data.get('tenant_id', 'Unknown')}. Estimated input tokens: {total_input_tokens}")
    ...
    @retry(
        ...
        wait=wait_exponential(multiplier=1, min=2, max=5),  # Exponential backoff: 2s, 4s
    ...
    def call_xai():
        ...
        payload = {
            ...
            "model": "grok-3-fast-beta",  # Switch to faster model
            "stream": False,
            "temperature": 0.5,
            "max_tokens": 500
        }
