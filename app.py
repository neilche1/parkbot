...
RENT_MANAGER_REPORT_URL = "https://shadynook.api.rentmanager.com/APIFunctions/Report/Get"
...
def authenticate_with_rent_manager():
    ...
    try:
        ...
        print(f"Rent Manager Auth Response Status: {response.status_code}")
        print(f"Rent Manager Auth Response Text: {response.text}")
        print(f"Rent Manager Auth Response Headers: {response.headers}")
        response.raise_for_status()
        # The response is a string (the token), not JSON
        RENT_MANAGER_API_TOKEN = response.text.strip('"')  # Remove surrounding quotes if present
        if not RENT_MANAGER_API_TOKEN:
            print("Error: Empty token received from Rent Manager")
            return None
...
# Fetch tenant data from Rent Manager API using a report
def fetch_tenants_from_rent_manager():
    ...
    # Request body for the Tenants report
    report_data = {
        "ReportType": "Tenants",  # Name of the report
        "Reports": "Tenants",     # Specific report to run
        "Separate": False,        # Combine results into a single dataset
        "NoContent": False,       # Include content in the response
        "ReturnCSVStream": True   # Return data as CSV stream (we'll parse it)
    }
    try:
        print(f"Fetching tenants with headers: {headers}")
        print(f"Request body: {report_data}")
        response = requests.post(RENT_MANAGER_REPORT_URL, headers=headers, json=report_data)
        print(f"Tenant Fetch Response Status: {response.status_code}")
        print(f"Tenant Fetch Response Text: {response.text}")
        print(f"Tenant Fetch Response Headers: {response.headers}")
        # Handle token expiration (401 Unauthorized)
        if response.status_code == 401:
            print("API token expired or invalid. Re-authenticating...")
            authenticate_with_rent_manager()
            headers["X-RM12-Api-Token"] = RENT_MANAGER_API_TOKEN
            print(f"Retrying tenant fetch with new headers: {headers}")
            response = requests.post(RENT_MANAGER_REPORT_URL, headers=headers, json=report_data)
            print(f"Tenant Fetch Retry Response Status: {response.status_code}")
            print(f"Tenant Fetch Retry Response Text: {response.text}")
            print(f"Tenant Fetch Retry Response Headers: {response.headers}")
        
        response.raise_for_status()
        # The response is a CSV stream; parse it
        csv_data = response.text
        tenants = {}
        # Split CSV into lines
        lines = csv_data.splitlines()
        if not lines:
            print("No tenant data returned from Rent Manager")
            return tenants
        ...
        for line in lines[1:]:
            ...
            if len(values) < 3:  # Ensure we have enough columns
                continue
            # Extract name and unit (adjust indices based on actual CSV structure)
            name = values[0].strip('"')  # Assuming first column is name
            try:
                first_name, last_name = name.split(" ", 1)
            except ValueError:
                first_name = name
                last_name = ""
            lot = values[1].strip('"') if len(values) > 1 else "Unknown"  # Assuming second column is unit
            balance = values[2].strip('"') if len(values) > 2 else "0.00"  # Assuming third column is balance
            due_date = values[3].strip('"') if len(values) > 3 else "1st"  # Assuming fourth column is due date
...
def identify_tenant(input_text):
    ...
    for tenant_key in TENANTS:
        ...
        if input_text in first_name_lower or input_text in last_name_lower:
            possible_matches.append(tenant_key)
